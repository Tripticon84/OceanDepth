...
void use_item(Inventory* inv, int index) {
    if (inv == NULL || index < 0 || index >= inv->nb_objets) {
        return;
    }

    Item* item = &inv->objets[index];
    
    // Vérifier que c'est un consommable
    if (item->type.type != CONSUMABLE) {
        return;
    }

    // Vérifier qu'il reste des items
    if (item->quantity <= 0) {
        return;
    }

    // Accéder au joueur global depuis game.h
    if (player == NULL) {
        return;
    }

    // Appliquer l'effet selon le type
    if (strcmp(item->effect, "boost_oxygene") == 0) {
        // Augmenter le maxOxygen de 40 points
        player->maxOxygen += 40;
        // Restaurer aussi l'oxygène actuel au nouveau max
        player->oxygen = player->maxOxygen;
    } else if (strcmp(item->effect, "soin") == 0) {
        // Restaurer la santé de 50 points (ou jusqu'au max)
        int healAmount = 50;
    ...
    } else if (strcmp(item->effect, "boost_vitesse") == 0) {
        // Réduire la fatigue (améliorer la vitesse)
        if (player->tiredness > 1) {
            player->tiredness--;
        }
    } else if (strcmp(item->effect, "cure_poison") == 0) {
        // Guérir le poison (si implémenté plus tard)
    ...
    // Si la quantité atteint 0, retirer l'item de l'inventaire
    if (item->quantity <= 0) {
        // Décaler tous les items suivants vers la gauche
        for (int i = index; i < inv->nb_objets - 1; i++) {
            inv->objets[i] = inv->objets[i + 1];
        }
        inv->nb_objets--;
    }
}
