...
void handle_map_input(void) {
    ...
    if (key == 'r' || key == 'R') {
        player->zoneIndex = 0;
        player->caseIndex = 0;
        // Restaurer l'oxygène au max à la surface
        player->oxygen = player->maxOxygen;
        // Restaurer la santé au max à la base
        if (player->caseIndex == 0) {
    ...
    if (is_case_zone_visible(newZone, newCase) || gameMap.zones[newZone].cases[newCase].isDiscovered) {
        ...
        // Restaurer l'oxygène au max à la surface (zoneIndex == 0)
        if (newZone == 0) {
            player->oxygen = player->maxOxygen;
        }
        // Restaurer la santé au max à la base (zoneIndex == 0 && caseIndex == 0)
        if (newZone == 0 && newCase == 0) {
            player->health = player->maxHealth;
        }
    }
}
